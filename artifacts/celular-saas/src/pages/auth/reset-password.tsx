...
import { Crown, KeyRound, ArrowLeft, CheckCircle2 } from "lucide-react";
import heroImage from "@assets/9FE3B637-3BED-471F-98A6-8CD90C1D69E5_1777058540929.jpeg";

const schema = z
  .object({
    senha: z.string().min(6, "A senha precisa ter pelo menos 6 caracteres"),
    confirmar: z.string().min(6, "A senha precisa ter pelo menos 6 caracteres"),
  })
...
export default function ResetPassword() {
  ...
  async function onSubmit(values: z.infer<typeof schema>) {
    if (!token) {
      toast.error("Link inválido. Solicite um novo email de redefinição.");
      return;
    }
    setLoading(true);
    try {
      ...
      toast.error(messageFromError(err as ApiError, "Erro ao redefinir a senha."));
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="mx-auto w-full max-w-md lg:w-[26rem] rounded-3xl border border-white/20 bg-black/55 p-6 shadow-2xl backdrop-blur-xl">
          ...
          {!token ? (
            <div className="space-y-4 text-white">
              <p className="text-sm">Link inválido ou ausente. Por favor, solicite um novo email de redefinição.</p>
              <Link href="/esqueci-senha" className="inline-flex items-center gap-2 text-sm font-semibold text-[hsl(28,85%,55%)] hover:underline">
                Solicitar novo link
              </Link>
            </div>
          ) : redefinido ? (
            <div className="space-y-5 text-white">
              <div className="flex items-center gap-2 rounded-xl border border-emerald-400/30 bg-emerald-400/10 p-4">
                ...
                <div className="text-sm leading-snug">
                  Senha redefinida com sucesso. Te levamos pra tela de login...
                </div>
              </div>
            </div>
          ) : (
            <>
              ...
              <p className="mt-3 text-sm leading-6 text-white/75">
                Escolha uma senha forte. Você poderá entrar imediatamente em seguida.
              </p>
